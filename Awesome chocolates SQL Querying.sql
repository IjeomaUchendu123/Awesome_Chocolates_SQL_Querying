----- 1.) Shipment information with salesnames and categories of products
...
select date_format(s.SaleDate, '%M %Y') as "Month", sum(s.Amount) "Sales", sum(s.Boxes) "Boxes"
FROM sales s
JOIN products pr on pr.PID = s.PID
...
----- 8.) Barr Shipments alone with JOIN clause

select s.SaleDate, s. Amount, p.Salesperson, p.Team
from sales s
JOIN people p ON s.SPID = p.SPID
and p.Salesperson = "Barr Faughny";
